...
В текущей схеме отдельной таблицы `balances` нет. Предполагается рассчитывать баланс из двух источников:

- начисления по обработанным заказам из `orders.accrual`, где `status = 'PROCESSED'`
- списания из `withdrawals.amount`

## Транзакционность `POST /api/user/balance/withdraw`

Списание баллов выполняется внутри транзакции.

Порядок работы такой:

1. В начале транзакции выбираются строки `orders` текущего пользователя, которые участвуют в расчете начисленного баланса, с `SELECT ... FOR UPDATE`
2. Затем в этой же транзакции рассчитывается доступный баланс:
   - начисления - сумма `orders.accrual` для заказов со статусом `PROCESSED`
   - списания - сумма `withdrawals.amount`
3. Если доступных средств недостаточно, транзакция откатывается
4. Если средств хватает, в таблицу `withdrawals` добавляется запись о новом списании, после чего транзакция фиксируется

Такой подход защищает от ситуации, когда два запроса на списание приходят одновременно. Первый запрос блокирует выбранные строки `orders`, поэтому второй не сможет пройти расчет баланса параллельно и будет ждать завершения первой транзакции. В результате списания для одного пользователя фактически выполняются последовательно, и одна и та же сумма не может быть списана дважды.
