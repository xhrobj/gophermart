...
> Момент, который выглядит как бизнес-косяк: если пользователь ошибётся при вводе номера заказа и отправит номер, который корректен по алгоритму Луна, но которого нет (и не будет) во внешнем `accrual`, такой заказ в текущей модели будет опрашиваться бесконечно долго - `gophermart` будет "вечно", с опредееленной периодичностью, запрашивать его статус в надежде, что он появится во внешней системе.
